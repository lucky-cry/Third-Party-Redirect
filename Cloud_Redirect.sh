...
# Cloud Warehouse: https://github.com/lucky-cry/Third-Party-Redirect
...
do
	...
	if [[ ! -z $(which curl) ]]; then
		...
		until [[ -f $MODDIR/Redirect.prop ]]; do
			curl -O 'https://raw.githubusercontent.com/lucky-cry/Third-Party-Redirect/main/Redirect.prop' >/dev/null 2>&1
			sleep 2
		done
	elif [[ ! -z $(which wget) ]]; then
		curlwget="wget"
		until [[ -f $MODDIR/Redirect.prop ]]; do
			wget 'https://raw.githubusercontent.com/lucky-cry/Third-Party-Redirect/main/Redirect.prop' >/dev/null 2>&1
			sleep 2
		done
	fi
	...
	if [[ "$A" -lt "$B" ]]; then
		...
		until [[ -f $MODDIR/Run_Redirect.sh ]]; do
			if [[ $curlwget == "curl" ]]; then
				curl -O 'https://raw.githubusercontent.com/lucky-cry/Third-Party-Redirect/main/Run_Redirect.sh' >/dev/null 2>&1
			elif [[ $curlwget == "wget" ]]; then
				wget 'https://raw.githubusercontent.com/lucky-cry/Third-Party-Redirect/main/Run_Redirect.sh' >/dev/null 2>&1
			fi
			sleep 3
		done
